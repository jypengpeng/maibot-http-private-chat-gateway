...
# 外部 HTTP 会话串行锁（避免同一 session_id 并发请求导致回复串扰）
_external_session_locks: Dict[str, asyncio.Lock] = {}


def _get_external_session_lock(session_id: str) -> asyncio.Lock:
    lock = _external_session_locks.get(session_id)
    if lock is None:
        lock = asyncio.Lock()
        _external_session_locks[session_id] = lock
    return lock


def _verify_external_api_token(x_maibot_token: Optional[str], authorization: Optional[str]) -> None:
    """校验外部 HTTP 私聊接口的最简鉴权（Header Token）"""
    allowed_tokens = [str(t).strip() for t in (global_config.maim_message.auth_token or []) if str(t).strip()]

    # 为避免误暴露公网接口，未配置 token 时直接拒绝
    if not allowed_tokens:
        raise HTTPException(status_code=503, detail="外部私聊接口未配置鉴权 token，请在 maim_message.auth_token 中配置")

    token = (x_maibot_token or "").strip()
...
class ExternalPrivateChatRequest(BaseModel):
    """外部 HTTP 私聊请求（最小可用版）"""

    session_id: str
    text: str = ""
    image_urls: List[str] = []
    user_name: Optional[str] = "外部用户"
    timeout_seconds: Optional[int] = 60


class ExternalPrivateChatResponse(BaseModel):
    """外部 HTTP 私聊响应（最小可用版）"""
...
def _build_external_user_id(session_id: str) -> str:
    """根据会话ID构造稳定用户ID，确保上下文复用"""
    return f"api_session_{session_id}"


def _download_image_to_base64_sync(url: str, timeout: int = 12) -> str:
    """同步下载图片 URL 并转换为 base64 字符串"""
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "MaiBot-HTTP-Bridge/1.0",
        },
        method="GET",
    )
    with urllib.request.urlopen(req, timeout=timeout) as resp:
        ...
        if content_type and "image" not in content_type:
            raise ValueError(f"URL 非图片资源: {url}, content-type={content_type}")
        data = resp.read()
        if not data:
...
async def _download_image_to_base64(url: str, timeout: int = 12) -> str:
    """异步包装：下载图片 URL 并转换为 base64"""
    return await asyncio.to_thread(_download_image_to_base64_sync, url, timeout)


def _build_external_private_message_data(
    session_id: str,
    text: str,
    ...
    """构造外部 HTTP 私聊请求对应的统一消息结构（私聊）"""
    if message_id is None:
        message_id = str(uuid.uuid4())

    user_id = _build_external_user_id(session_id)
    platform = "api"
    ...
    if text and text.strip():
        ...
        seg_items.append({"type": "text", "data": normalized_text})
        plain_parts.append(normalized_text)

    for image_base64 in image_base64_list:
        seg_items.append({"type": "image", "data": image_base64})

    # 与 WebUI 保持一致，显式标注 at_bot
    seg_items.append({"type": "mention_bot", "data": "1.0"})

    processed_plain_text = " ".join(plain_parts).strip()
...
async def _wait_bot_replies_for_chat(
    chat_id: str,
    since_ts: float,
    timeout_seconds: int = 60,
    poll_interval: float = 0.35,
) -> List[str]:
    """等待指定 chat_id 的机器人回复文本"""
    ...
    while time.time() < deadline:
        rows = (
            ...
            .where(
                ...
                & (Messages.user_id == bot_user_id)
                & (Messages.time >= since_ts)
            )
            .order_by(Messages.time.asc())
        )
        reply_texts = [(m.processed_plain_text or m.display_message or "").strip() for m in rows]
        reply_texts = [t for t in reply_texts if t]

        if reply_texts:
            if first_hit_ts is None:
                first_hit_ts = time.time()
            elif (time.time() - first_hit_ts) >= settle_window:
                return reply_texts
...
@router.post("/external/private/message", response_model=ExternalPrivateChatResponse)
async def external_private_message(
    request: ExternalPrivateChatRequest,
    x_maibot_token: Optional[str] = Header(default=None, alias="X-MaiBot-Token"),
    authorization: Optional[str] = Header(default=None),
):
    """外部 HTTP 私聊入口（内网最小可用版，无鉴权）"""
    _verify_external_api_token(x_maibot_token, authorization)

    session_id = (request.session_id or "").strip()
    if not session_id:
        return ExternalPrivateChatResponse(
            success=False,
            session_id="",
            request_message_id="",
            reply_text="session_id 不能为空",
            reply_messages=[],
            timeout=False,
        )

    text = (request.text or "").strip()
    image_urls = [u.strip() for u in (request.image_urls or []) if isinstance(u, str) and u.strip()]
    if not text and not image_urls:
        return ExternalPrivateChatResponse(
            success=False,
            session_id=session_id,
            request_message_id="",
            reply_text="text 与 image_urls 不能同时为空",
            reply_messages=[],
            timeout=False,
        )
    ...
    user_name = (request.user_name or "外部用户").strip() or "外部用户"
    session_lock = _get_external_session_lock(session_id)

    try:
        async with session_lock:
            image_base64_list: List[str] = []
            for url in image_urls:
                image_base64_list.append(await _download_image_to_base64(url))

            request_message_id = str(uuid.uuid4())
            message_data = _build_external_private_message_data(
                session_id=session_id,
                text=text,
            ...
            # 先计算目标私聊 chat_id，再注入消息
            target_user_id = _build_external_user_id(session_id)
            target_chat_id = get_chat_manager().get_stream_id(platform="api", id=target_user_id, is_group=False)
            since_ts = time.time()

            await chat_bot.message_process(message_data)
            replies = await _wait_bot_replies_for_chat(
        ...
        logger.error(f"外部 HTTP 私聊处理失败: {e}")
        return ExternalPrivateChatResponse(
            success=False,
            session_id=session_id,
            request_message_id="",
            reply_text=f"处理失败: {str(e)}",
            reply_messages=[],
            timeout=False,
        )
